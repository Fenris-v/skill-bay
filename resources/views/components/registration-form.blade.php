<form class="form Authorization" action="{{route('user.store')}}" method="post">
	...
	<div class="row">		  
		<div class="row-block">
			...
			<x-user-field type="password"  name="passwordReply" title="{{__('user_messages.placeholder_password_reply')}}" id="passwordReply" placeholder="Введите пароль повторно">{{__('user_messages.password_confirm')}}</x-user-field>
