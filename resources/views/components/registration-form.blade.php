<form class="form Authorization" action="{{route('user.store')}}" method="post">
	...
	<div class="row">		  
		<div class="row-block">
			...
			<x-user-field type="email"  name="email" title="{{__('user_messages.email')}}" id="mail" placeholder="{{__('user_messages.placeholder_mail')}}">{{__('user_messages.email')}}</x-user-field>
