...
<form class="form Authorization" action="{{route('reset-password-send')}}" method="post">
	...
	<div class="row">
		<div class="row-block">
			...
			<x-user-field type="password"  name="password_confirmation" title="{{__('user_messages.password_confirm')}}" id="password_confirmation" placeholder="{{__('user_messages.placeholder_password_reply')}}">{{__('user_messages.password_confirm')}}</x-user-field>
